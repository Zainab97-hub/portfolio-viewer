...
import adminMiddleware from "../middlewares/adminMiddleware.js"; // Importiert die Admin-Middleware, um zu überprüfen, ob der Benutzer Admin-Rechte hat.
import multer from 'multer'; // Importiert Multer, um Datei-Uploads zu verwalten.
import path from 'path'; // Importiert das Path-Modul, um Dateipfade zu handhaben.
import fs from 'fs'; // Importiert das File-System-Modul, um mit dem Dateisystem zu arbeiten.
...
// Route: Holt das Profil des angemeldeten Benutzers
...
router.get('/years', adminMiddleware, async (req, res) => {
...
router.get('/data/year/:year', adminMiddleware, async (req, res) => {
...
router.get('/user/:userId/pins', adminMiddleware, async (req, res) => {
...
router.get('/check-year/:year', adminMiddleware, async (req, res) => {
  const { year } = req.params;
...
// Route: Überprüft, ob ein Name existiert
router.get('/check-name/:name', adminMiddleware, async (req, res) => {
...
// Route: Überprüft, ob eine Beschreibung existiert
router.get('/check-description/:description', adminMiddleware, async (req, res) => {
...
router.get('/check-pin-data/:id', adminMiddleware, async (req, res) => {
  const { id } = req.params;
  const query = `
    SELECT 
...
router.post('/add-year', adminMiddleware, async (req, res) => {
  const { year } = req.body;
...
router.get('/get-year-id/:year', adminMiddleware, async (req, res) => {
  const { year } = req.params;
...
router.post('/add-pin/:userId', adminMiddleware, upload.fields([
...
router.put('/edit-pin/:id', adminMiddleware, upload.fields([
  { name: 'image', maxCount: 1 },
  { name: 'video', maxCount: 1 },
  { name: 'pdf', maxCount: 1 },
  { name: 'panorama', maxCount: 1 },
...
router.delete('/delete-pin/:id', adminMiddleware, async (req, res) => {
  const { id } = req.params;
...
// Route: Löscht ein Jahr und die zugehörigen Portfolios
router.delete('/delete-year/:year', adminMiddleware, async (req, res) => {
  const { year } = req.params;

  const getYearIdQuery = 'SELECT id FROM years WHERE year = ?';
  const getFileUrlsQuery = `
    SELECT 
      i.image_url, 
      v.video_url, 
  ...
  try {
    const yearResults = await executeQuery(getYearIdQuery, [year]);
    if (yearResults.length === 0) {
      return res.status(404).json({ message: 'Year not found' });
    }
    const year_id = yearResults[0].id;

    const fileResults = await executeQuery(getFileUrlsQuery, [year_id]);
    ...
    const deletePortfoliosQuery = 'DELETE FROM portfolios WHERE year_id = ?';
    const deleteUrlQuery = 'DELETE FROM url WHERE work_id IN (SELECT id FROM portfolios WHERE year_id = ?)';

    await executeQuery(deletePortfoliosQuery, [year_id]);
    await executeQuery(deleteUrlQuery, [year_id]);

    const deleteYearQuery = 'DELETE FROM years WHERE id = ?';
    await executeQuery(deleteYearQuery, [year_id]);
...
router.get('/search-suggestions', adminMiddleware, async (req, res) => {
...
router.get('/check-title/:title', adminMiddleware, async (req, res) => {
...
router.get('/users-with-works', adminMiddleware, async (req, res) => {
  const query = `
